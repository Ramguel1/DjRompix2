...
guardar.onclick=()=>{
  ...
  if (nombre.trim()==""||ap.trim()==""||am.trim()=="") {
    Swal.fire ({
      title: "Alumnos",
      text:"Tienes campos vacios, ingresa valores validos",
      icon:"error"
    });
    return;
  }
...
const imprimirAlumnos = ()=>{
  var x=
  (JSON.parse(localStorage.getItem("alumnos"))==null)?
  ...
  x.forEach(a=>{
    ...
    <td><button class="btn btn-warning" onclick="mostrarAlumnos(${index})" data-bs-toggle="modal" data-bs-target="#actualizarR">Editar</button></td>
    </tr>
    `
    index++;
  })
...
delAlumnos=(index)=>{
  Swal.fire({
    title:"¿Estas seguro de eliminar a este alumno?",
    showDenyButton:true,
    confirmButtomText:"Si",
    denuButtonText:"No"
  }).then((result)=>{
    if (result.isConfirmed) {
      ...
      Swal.fire("El alumno se elimino exitosamente","","success");
    }
  })
}
...
actualizar.onclick = () =>{
 ...
 var myModal = new bootstrap.Modal(document.getElementById(`actualizaR`));
 myModal.hide();
